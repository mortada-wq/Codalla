...
export default function GitHubTab({ onClone: _onClone }: GitHubTabProps = {}) {
  ...
  const handleCloneRepo = async (repoUrl: string) => {
    if (!ready || !importChat) {
      toast.error('Clone not ready — open a chat first');
      return;
    }
    ...
    try {
      ...
      const filesMessage: Message = {
        role: 'assistant',
        content: `Cloning the repo ${repoUrl} into ${workdir}\n\n<boltArtifact id="imported-files" title="Git Cloned Files" type="bundled">\n${fileContents.map((f) => `<boltAction type="file" filePath="${f.path}">\n${escapeBoltTags(f.content)}\n</boltAction>`).join('\n')}\n</boltArtifact>`,
        id: generateId(),
        createdAt: new Date(),
      };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(isReposExpanded ? stats.repos : stats.repos.slice(0, 12)).map((repo) => (
                      <RepositoryCard
                        ...
                        showHealthScore
                        showExtendedMetrics
                        onSelect={() => handleCloneRepo(repo.clone_url)}
                      />
                    ))}
                  </div>
