  ...
  const renderDetails = (details: any) => {
    ...
    if (log.category === 'api') {
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            <span className={details.method === 'GET' ? 'text-blue-400' : 'text-blue-500'}>{details.method}</span>
            <span>•</span>
...
export function EventLogsTab() {
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with Yalla.Sahib
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('Yalla.Sahib - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by Yalla.Sahib', margin, doc.internal.pageSize.getHeight() - 10);
